...
Worker.prototype.runServer = function (config) {
    ...
    var httpRequestHandler = function (req, res) {
        ...
        var proxyRequest = function () {
            this.cache.getBackendFromRequest(req.headers.host, req.url, function (err, code, backend) {
                if (err) {
                ...
                if (backend.frontendUrl.length) {
                    req.url = req.url.substring(backend.frontendUrl.length);
                }
