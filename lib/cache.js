 ...
 * error.
 */
Cache.prototype.getBackendFromRequest = function (host, requrl, callback) {
    if (host === undefined) {
    ...
    readFromCache(host, function (rows) {
        ...
        // Parse and sort backends
        var parsedBackends = [];
        backends.forEach(function (bcd, idx) {
            var parsed = url.parse(bcd);
            parsed.id = idx;
            parsed.frontendUrl = (parsed.hash || '#').substring(1);
            parsedBackends.push(parsed);
        });
        parsedBackends.sort(function (a, b) {
            return b.frontendUrl.length - a.frontendUrl.length;
        });

        // Match with URI prefix and filter out dead backends
        var maxPrefix = 0;
        var selected = [];
        parsedBackends.forEach(function (bcd, idx) {
            if (path === bcd.frontendUrl || path.indexOf(bcd.frontendUrl+'/') === 0) {
                if (bcd.frontendUrl.length >= maxPrefix) {
                    maxPrefix = bcd.frontendUrl.length;
                    if (deads.indexOf(bcd.id) === -1) {
                        selected.push(bcd);
                    }
                }
            }
        ...
        var backend = selected[Math.floor(Math.random() * selected.length)];
